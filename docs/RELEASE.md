...
1. `npm run data:build && npm run validate:quran && npm run validate:data`
2. `npm test` (يشمل: مواقيت مرجعية، بحث القرآن، قاعدة المدن، الأذكار، المشاركة، وفحص واجهة شامل `tests/app-smoke.test.tsx`).
3. `npm run build`
4. `npm run icons` ثم `npm run cap:android` للتأكد من ظهور أيقونة «صليها» وشاشة البداية على الجهاز.
5. على الجهاز: مواقيت اليوم تطابق التوقيت المحلي، البحث عن آية يعمل بالكتابة بدون تشكيل، المشاركة النصية والصورية تعمل، عدّاد الأذكار يزيد بالضغط، والمشغّل يقفز ±١٠ ثوانٍ وله مؤشر تقدم.
